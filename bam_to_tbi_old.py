...
def convert_riboseq(options):
    ...
    sam_handle = pysam.Samfile(options.bam_file, "rb")
    fwd_handle = dict([(r,open(fwd_count_file+'.%d'%r, 'w')) for r in READ_LENGTHS])
    rev_handle = dict([(r,open(rev_count_file+'.%d'%r, 'w')) for r in READ_LENGTHS])
    ...
    for cname,clen in zip(sam_handle.references,sam_handle.lengths):
        ...
        fwd_counts = dict([(r,dict()) for r in READ_LENGTHS])
        rev_counts = dict([(r,dict()) for r in READ_LENGTHS])
        for read in sam_iter:

            # skip reads not of the appropriate length
            if read.rlen not in READ_LENGTHS:
                continue
        ...
        for r in READ_LENGTHS:
            indices = np.sort(list(fwd_counts[r].keys()))  # Added list()
            for i in indices:
                fwd_handle[r].write('\t'.join([cname, '%d'%i, '%d'%(i+1), '%d'%fwd_counts[r][i]])+'\n')

            indices = np.sort(list(rev_counts[r].keys()))  # Added list()
            for i in indices:
